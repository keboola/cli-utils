            ...
            foreach ($rows as $row) {
                ...
                if ($withConfiguration) {
                    ...
                    try {
                        ...
                        if ($purgeRefused) {
                            $output->writeln(sprintf(
                                '<error>WARNING: %s refused with "cannotDeleteConfiguration" for %s'
                                . ' — configuration likely still sits in trash and its workspace + backend user'
                                . ' still exist. Re-check and re-run later.</error>',
                                $actionDescription,
                                $description,
                            ));
                            $failed++;
